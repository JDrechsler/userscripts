...
function getAtlToken(){
  return 'BS26-60JM-900M-QWBU_0d69ba4911718ab6d946c0bf4bf79802e3935551_lin'
}
async function makePostRequest(currentJiraIssueKey, numStoryPoints){
      let resGet = await fetch(`https://atc.bmwgroup.net/jira/rest/greenhopper/1.0/xboard/issue/details.json?rapidViewId=29195&issueIdOrKey=${currentJiraIssueKey}`);
      ...
      if(dataGet === undefined){
          ...
          let res = await fetch("https://atc.bmwgroup.net/jira/secure/DetailsViewAjaxIssueAction.jspa?decorator=none", {
              "headers": {
                  "accept": "*/*",
              ...
              "referrer": "https://atc.bmwgroup.net/jira/secure/RapidBoard.jspa?rapidView=29195&projectKey=CHGFRWRDUS&view=planning&selectedIssue=CHGFRWRDUS-430&issueLimit=100",
...
async function addStoryPointsToDom(numStoryPoints){
      ...
      firstChildClone.firstElementChild.firstElementChild.textContent = 'Story Points:'
      ...
      if(storyPointsValEl !== undefined){
          storyPointsValEl.addEventListener("click", async function(){
              let newStoryPointsVal = prompt(`Please enter new story point value for ${getCurrentJiraIssueKey()}`);
...
async function makeGetRequest(currentJiraIssueKey){
  let res = await fetch(`https://atc.bmwgroup.net/jira/rest/greenhopper/1.0/xboard/issue/details.json?rapidViewId=29195&issueIdOrKey=${currentJiraIssueKey}`);
  let data = await res.json()
  if(data.key === undefined){
      ...
      let storyPointsObj = tabHeader.fields.find(field =>
